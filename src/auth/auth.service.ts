...
export class AuthService {
    ...
    async validatePassword(password: string, salt: string, storedPassword: string): Promise<boolean> {
    ...
    async createJwtAccessToken(payload:JwtPayload):Promise<{accessToken:string}>{
        const accessToken= await this.jwtService.sign(payload);
        return {accessToken}
    }

    async validateUserCredentials(logInDto: LogInDto): Promise<{accessToken: string}> {
